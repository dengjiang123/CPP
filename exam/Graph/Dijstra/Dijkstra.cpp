...
Graph_DG::Graph_DG(int vernum, int edgenum) {
	...
	arc = new int* [edgenum];
	farc = new int* [edgenum];
	fpath = new int* [edgenum];
	for (int i = 0; i < vernum; i++) {
		arc[i] = new int[vernum];
		farc[i] = new int[edgenum];
		fpath[i] = new int[vernum];
...
void Graph_DG::Flord() {
	for (int k = 0; k < vernum; k++)
		for (int i = 0; i < vernum; i++)
			for (int j = 0; j < vernum; j++)
				if (farc[i][j] > farc[i][k] + farc[k][j] && farc[i][k] != INT_MAX && farc[k][j] != INT_MAX) {
					farc[i][j] = farc[i][k] + farc[k][j];
					fpath[i][j] = k + 1;
				}
				else if (i == j)
					farc[i][j] = 0;
...
void Graph_DG::Print_Flord(int begin) {
	for (int i = 100; i <= vernum; i += 100) {
		//std::cout << begin << " to " << i + 1 << " path :v" << begin;
		std::cout << "v" << begin;
		Flord_Temp(begin, i);
		if (farc[begin - 1][i - 1] != INT_MAX)
			std::cout << "         " << farc[begin - 1][i - 1];
		else
			std::cout << "是无最短路径的";
		std::cout << std::endl;
	}
...
void Graph_DG::Flord_Temp(int begin, int end) {
	if (fpath[begin - 1][end - 1] >= 0) {
		Flord_Temp(begin, fpath[begin - 1][end - 1]);
		Flord_Temp(fpath[begin - 1][end - 1], end);
	}
	else
		std::cout << "-->v" << end;
}
